...
class Dequeue:
  ...
  def push_front(self, data):
    n = Node(data)
    if not self.first: self.last = n
    else: n.next, self.first.prev = self.first, n
    self.first = n
  def push_back(self, data):
    n = Node(data)
    if not self.last: self.first = n
    else: n.prev, self.last.next = self.last, n
    self.last = n
  def pop_front(self):
    if not self.first: return
    a = self.first
    if self.first is not self.last: self.first.next.prev = None
    else: self.last = None
    self.first = a.next
    return a.data
#
...
#    return b
  def pop_back(self):
    ...
    a = self.last
    if self.first is not self.last: self.last.prev.next = None
